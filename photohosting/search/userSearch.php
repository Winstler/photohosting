...
if ($categorie == "0") {
    $q .= "(pubName LIKE '%$search%' OR pubDescription LIKE '%$search%' OR nickname LIKE '%$search%') AND userID = '$userID' ";
    $count = "SELECT COUNT(pubID) FROM publication WHERE (pubName LIKE '%$search%' OR pubDescription LIKE '%$search%' OR nickname LIKE '%$search%') AND userID = '$userID'";
} else {
    $q .= "tag = '$categorie' AND (pubName LIKE '%$search%' OR pubDescription LIKE '%$search%' OR nickname LIKE '%$search%') AND userID = '$userID' ";
    $count = "SELECT COUNT(pubID) FROM publication WHERE tag = '$categorie' AND (pubName LIKE '%$search%' OR pubDescription LIKE '%$search%' OR nickname LIKE '%$search%') AND userID = '$userID'";
}
...
<body>
    ...
    <div class="infoUp">
        <h1 style="margin-bottom: 0;">Пошук публікацій користувача</h1>
